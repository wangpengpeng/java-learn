package com.wpp.aleetcode.problem.heap;

import java.util.Comparator;
import java.util.PriorityQueue;

/**
 * Created by gouthamvidyapradhan on 06/04/2019
 ...
 *
 * Solution O(NLogN). The key insight to this problem is to notice that the point of a number
 * changes to 1 from 0 if the position changes to A.length - 1 and similarly the point changes to 1 from 0 if the
 * position changes to a index = NUM - 1.
 * Maintain a priority queue with rotation_count (the number of rotation required to change its points from either
 * 0 to 1 or from 1 to 0), pop all the indices from priority queue which has rotation_count equal to current rotation
 * count and update the rotation_count to its next value.
 * Maintain a max count and the rotation index pair and return rotation index as the answer.
 */
